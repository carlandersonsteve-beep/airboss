        function OrderCard({ order, customer, updateOrderStatus, addTicket, messages, addMessage, startOrderService, markOrderReadyForFrontDesk }) {
            ...
            const handleSaveAndNotify = (actualFuel, completionNotes) => {
                ...
                // Sync through centralized adapter
                syncAdapters.syncToSheets('updateOrder', { order: updatedOrder });
                
                // Update status in parent — pass all fields so React state stays in sync
                ...
                <div className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition">
                    ...
                    {order.departureDate && (
                        <div className="text-sm text-gray-600 mb-3 bg-green-50 border border-green-200 p-2 rounded">
                            <span className="font-medium">✈️ Departure:</span> {new Date(order.departureDate + 'T12:00:00').toLocaleDateString('en-US', {weekday:'short', month:'short', day:'numeric'})}
                            {order.departureTime && ` at ${(() => { const [h,m] = order.departureTime.split(':'); const ampm = h >= 12 ? 'PM' : 'AM'; return ((h % 12) || 12) + ':' + m + ' ' + ampm; })()}`}
                        </div>
                    )}
